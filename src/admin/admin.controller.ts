...
export class AdminController {
  ...
  async registerDevice (@Body() data:AdminDeviceRegisterDto, @Res() response:Response) {
  ...
  async sendCommand (@Body() data:AdminDeviceCommandDto, @Res() response:Response) {
  ...
  @ApiOperation({ summary: '3. 디바이스 가동 상태 불러오기 (10분) - (미세먼지, 온도, 습도 등)' })
  ...
  @ApiOperation({ summary: '4. 디바이스 명령 상태 불러오기 (10초) - (전원, 모드 등)' })
